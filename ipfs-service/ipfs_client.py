...
class IPFSClient:
    ...
    def _connect(self):
        try:
            if Config.IPFS_HOST.replace('.', '').isdigit():
                addr = f"/ip4/{Config.IPFS_HOST}/tcp/{Config.IPFS_PORT}/http"
            else:
                addr = f"/dns4/{Config.IPFS_HOST}/tcp/{Config.IPFS_PORT}/http"
            self.client = Client(addr=addr)
            self.client.id()
            logger.info(
                f"Connected to IPFS at {Config.IPFS_PROTOCOL}://{Config.IPFS_HOST}:{Config.IPFS_PORT}")
